...
3. 只執行當前 phase 允許的 build-task。
   ...
   非純文檔任務必須先通過 `menmery entry_turn(...)` 取得上下文與治理預覽，並遵循返回的 recommended call；若當前環境無法調用 `menmery`，必須明確標記為 fallback，而不是假裝已有長期上下文。
...
- 不得自行修改 North Star
- 不得跳過 `28-product-principles.md` 重新解釋產品目標
- 不得在未批准下擴大 scope
- 不得跳過 gate
