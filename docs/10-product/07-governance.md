...
> truth/governance runtime. It may consume sibling governance or routing
> services when useful, but its core gate semantics must remain valid with local
> evidence and local risk mapping.
>
...
| Concern | Owner |
|---------|-------|
| local gate semantics / risk mapping | `dev_master` |
| local evidence retention | `dev_master` |
| durable memory / external audit sink | `menmery` when used |
| action level / approval lane | `menmery` when used |
| LLM routing execution / runtime planner / model failover | `auto_router` when used |
| routing feedback / low-risk learner optimization | `auto_router` when used |
...
- changes to [autonomy-ratchet.md](../20-current-window/autonomy-ratchet.md),
  including thresholds, hard red lines, tier rules, or metric windows
...
- runner facts digest
- verifier decision
- optional external governance or audit reference when used
...
- a heavy competing truth/governance platform created as a side effect of Phase 1
- an approval controller outside the current gate discipline
