...
> are restated here and reconciled with `docs/30-integrations/27-external-systems-boundary.md`.
...
The current working calibration, based on April 2026 human direction, is:

| Tier | Meaning | Product stance |
|------|---------|----------------|
| `L1` | idea to clickable prototype or bounded repo-local artifact | Target for full automation after the kernel is proven. |
| `L1.5` | queued 24/7 execution with bounded slices, verifier, evidence, async human review, and multi-project operation | Near-term target operating model. |
| `L2` | real product / SaaS work involving accounts, payments, multi-tenancy, security, release, or production concerns | Semi-automatic only: AI prepares changes, PRs, reports, risk labels, and recommendations; humans approve key decisions. |
| `L2 full-auto` | unattended real SaaS/product delivery without human governance | Not a product target under current capability assumptions. |
| `L3` | high-compliance, high-integration, or incident/operations-heavy work | AI assists and verifies; humans retain decision ownership. |

This calibration does not shrink the product roadmap. It defines the autonomy
boundary. The practical target is:

```text
24/7 L1 product incubation factory
+ L2 semi-automatic product assistant
+ human-governed decisions for security, release, business, and autonomy expansion
```

Long-running AI work must be anchored by bounded slices, tests, verifier output,
durable evidence, and human or truth-store checkpoints. A model's self-report is
not enough evidence for multi-hour or production-impact work.

The mechanism for widening or tightening autonomy is
[autonomy-ratchet.md](../20-current-window/autonomy-ratchet.md). Capability
growth should come from append-only evidence records, not from quiet assumption
changes.

Implementation-time boundaries are defined in
[engineering-execution-boundaries.md](engineering-execution-boundaries.md).
Those boundaries distinguish non-relaxing responsibility red lines from
ratchetable execution guardrails.
...
structured facts. `docs/10-product/CONTRACTS.md` is the authority for this layer.
...
Long-term context, external audit sinks, and governed action may use `menmery`
when that service is useful. The local bounded loop must still remain valid
without it.

`menmery` owns, when used:
...
## 28.10 Control Plane Ownership

`dev_master` owns software-delivery scheduling, authorization, governance,
evidence, cost, review state, and autonomy changes.

Codex, Claude Code, API models, `auto_router`, and future agent systems are
workers or integrations. They may propose slices and execute authorized work,
but they must not own the queue, final approval, ratchet, cost ceilings, or
admissible evidence.

In governed workspaces, paid AI calls and repo mutations intended to count as
evidence must route through `devmasterd`. Direct provider calls can be useful
for informal exploration, but they are not admissible evidence unless imported
or re-run through the governed provider-adapter path.
