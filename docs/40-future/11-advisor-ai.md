...
> **分層狀態：Deferred Program（藍圖層）。** 本文件中的 Advisor 能力不是當前 active core 的實作承諾，只保留為延後接入的制度設計。當前倉庫尚未重建 Advisor 主動提案流，所有提案模板、執行動作、批准流程都是啟用時的目標形態。

> **本部分是 v3 核心新增**。v2 的 AI 只在代碼質量上做決策。v3 的目標是讓 AI 參與更廣泛的技術決策——不是替代人，而是確保每個決策都有數據，而不是直覺。當前該決策參與面尚未實現。
>
> **注意**：本文件中所有代碼塊、提案模板、自動執行動作都是 Advisor AI 啟用時的目標形態。實際實現需基於 StateStore + gateway_client 重寫。
