...
`dev_master` is responsible, when this integration is in use, for:
...
internal imports. This file describes an optional service-consumption path, not
current mandatory runtime behavior.
...
For every routed stage that actually uses the gateway, local records should preserve:
...
When a run uses routed execution, local request/result/feedback correlation
should emit a structured event and follow `docs/10-product/CONTRACTS.md`.
