...
- `service_context`: 是否使用 `menmery` / `auto_router` 等外部服務、使用了哪些公開接口、以及本地 fallback 標記

限制：
- 非純文檔 build-task 必須先聲明本地風險判斷、是否需要外部服務、以及服務不可用時如何退化
- `orchestration` build-task 不得包含 repo mutation 或 shell execution
- `execution` build-task 不得包含 final approval decision
- `evidence` build-task 必須 append-only；若使用外部 evidence sink，需記錄回寫 reference 或 pending 狀態
- `docs` build-task 不得把 deferred / future 能力改寫為 active 承諾
- 不得用 build-task 自建重型、競爭性的 truth/governance/router 子系統
