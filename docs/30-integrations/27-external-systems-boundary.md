...
|------|-----------------------------|
| durable personal/project context | `menmery` when that service is used |
| canonical evidence / audit / action lane | `menmery` when that service is used |
| repo mutation, build, test, scan | `dev_master` external harness / worker |
| runner facts and verifier expectations | `dev_master` |
| LLM backend selection and failover | `auto_router` when routed execution is used |
| routing feedback and learner artifacts | `auto_router` when routed execution is used |
| product-level delivery governance | `dev_master`, optionally augmented by `menmery` when used |
| product-level model requirements | `dev_master`, coordinated with `auto_router` when used |
...
- `dev_master` must hard-depend on `menmery` or `auto_router` for every bounded run.
