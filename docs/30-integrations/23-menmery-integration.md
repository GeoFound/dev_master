...
`menmery` is long-term cognitive infrastructure. For `dev_master`, it is an
optional service integration for durable context, memory, audit, governance
preview, and evidence sinks when that service materially improves the current
task.
...
For non-doc software-change work, `entry_turn` is the recommended path when the
run benefits from long-term context, missing-side detection, or governance
preview. It is not a universal precondition for every bounded change. Local
runs that do not need `menmery` remain valid if they leave append-only local
evidence and clearly mark the service as unused or unavailable.

```text
1. decide whether the task needs `menmery`
2. if yes, call `entry_turn(...)` and follow the returned recommended call when useful
3. run the current-window execution worker inside the bounded slice
4. verify runner facts, risk facts, checks, diff digest, and evidence refs
5. if `menmery` was used, write back through `remember(...)` or another
   documented surface; otherwise keep the result in local evidence
```
...
keep local evidence until writeback is possible or unnecessary. `structural_status=complete`
...
When used, `menmery` is the service integration point for:
...
| canonical evidence | hold durable evidence references and supersede chains when the run chooses that sink |
...
`dev_master` may use its own green/yellow/red labels for software-delivery risk.
When `menmery` returns an action level or approval lane, the stricter result
wins.
...
7. `menmery` is a useful sibling service, not a mandatory runtime prerequisite
   for every bounded task.
8. Product roadmap capabilities must not be erased to satisfy a current-window
   gate.
