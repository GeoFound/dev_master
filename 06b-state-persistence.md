...
| 優先級 | 後端 | 適用場景 | 說明 |
|--------|------|----------|------|
| **首選** | PostgreSQL（`localhost:5432`） | 工作站開發環境 | 共享開發 DB 已就位（dev/devpass/devdb），支持 ACID 事務、advisory lock、JSONB 查詢。審計記錄和治理狀態天然適合結構化存儲 |
| **備選** | 本地文件系統（JSONL + JSON） | PostgreSQL 不可用時 | 零依賴 fallback，追加寫入天然原子，但缺少事務和高效查詢 |
| **可選** | MCP 持久化服務 | 需要遠端訪問時 | 接口不變，通過 MCP server 封裝 PostgreSQL 或其他後端 |
...
鎖機制也可升級為 PostgreSQL advisory lock，避免文件鎖的 TTL 和死鎖問題。Redis（`localhost:6379`）可作為輔助鎖方案，適用於需要更細粒度超時控制的場景。
